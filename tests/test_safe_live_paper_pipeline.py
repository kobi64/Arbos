...
def test_profitable_live_opportunity_executes(pipeline):
...
def test_unprofitable_live_opportunity_does_not_execute(pipeline):
    result = pipeline.execute(
        opportunity=valid_opportunity(),
        starting_value=1000.0,
...
def test_opportunity_id_becomes_route_id(pipeline):
    ...
    assert result["route_id"] == "OPP-080"


def test_live_market_prices_are_used(pipeline):
    result = pipeline.execute(
        opportunity=valid_opportunity(),
        starting_value=1000.0,
        gross_final_value=1050.0,
...
def test_history_records_result(pipeline):
    pipeline.execute(
        opportunity=valid_opportunity(),
        starting_value=1000.0,
        gross_final_value=1050.0,
